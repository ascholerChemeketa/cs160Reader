...
!function($s) {
  ...
  var createDeviceController = function(device) {
    ...
    var layoutUI = function() {
	  ...
	  var isVertical = device.vertical;
	
      device.$ui.children('.simcir-device-body').
        attr({x: 0, y: 0, width: w, height: h});
	
      var pitch = device.halfPitch? unit / 2 : unit;
  	  var componentSideDimentsionLimit = isVertical ? w : h;
  	  var blankDimentsionLimit = isVertical ? h : w;
	
      var layoutNodes = function(nodes, maxValue) {
        var offset = (componentSideDimentsionLimit - pitch * (nodes.length - 1) ) / 2;
        $.each(nodes, function(i, node) {
			if(isVertical)
          		transform(node.$ui, pitch * i + offset, maxValue);
			else
          		transform(node.$ui, maxValue, pitch * i + offset);
        });
      };
	
      layoutNodes(getInputs(), 0);
      layoutNodes(getOutputs(), blankDimentsionLimit);
