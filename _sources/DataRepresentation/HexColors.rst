...
The first place many people encounter hexadecimal is learning about specifying colors for websites. Colors are generally specified as 24-bit numbers - 8 bits each to represent how much red, green and blue. These 8 bits are enough to give values of :math:`{0-255}_{10}` (:math:`{00000000-11111111}_{2}`) for each of the three primary colors. To make different colors, we mix different amounts of the three primaries. 255 red, 0 green, and 140 blue makes a :pink:`nice pink`. To specify this color, a website designer can write the 24 binary digits as 6 hex digits (remember that 4 binary digits correspond to one hex digit). 255 is FF in hex. 0 is 00 in hex. 140 is 8C in hex. So :pink:`this shade of pink` can be written as FF008C - a website designer would type something like "color: #FF008D;" to specify that some text should be pink.
