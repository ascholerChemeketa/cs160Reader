...
.. sidebar:: What's Happening?
    ...
    You can check what processes are currently running on your computer (and manage them) by pressing *ctrl-alt-esc* (Windows) or *command + option + escape* (OSX). On Windows then click "More details".
...
Hundreds of times a second, the hardware interrupts the running program and gives the operating system a chance to run. It also gets swapped into the processor to handle messages from the hardware (\ *"hey, the mouse was clicked"*). Each time the OS is being run it processes any events that need to be handled and then either restarts the program that was running or picks a program from the ready queue to be run next. By switching tasks every few milliseconds, the processor makes it look like it is doing multiple things at once.
  
.. figure:: Images/ProcessStateDiagram.swf
   :width: 550
   :height: 450

   `Animation used by permission of Virgnia Tech <http://courses.cs.vt.edu/csonline/>`__
   
Many CPUs feature multiple processing "cores" and so actually can be running instructions from two or more processes at once. But the basic model still holds - each processing core is constantly switching between different processes.