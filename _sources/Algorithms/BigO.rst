...
* Logarithmic : :math:`O(log(n))`
    Any algorithm where the relationship between work **f(n)** and input **n** is described by a logarithmic function. It could be :math:`f(n) = log_2(n)` or :math:`f(n) = 3 \cdot log_{10}(n)`. Note that we don't actually care what base the logarithm is.
    
    We will see an example of this category soon...
    
* Linear : :math:`O(n)`
    Any algorithm where the relationship between work **f(n)** and input **n** is described by a linear function. It could be :math:`f(n) = \frac{n}{2} - 4` or :math:`f(n) = 5n + 100`.
...
* Quadratic : :math:`O(n^2)`
    Any algorithm where the relationship between work **f(n)** and input **n** is given by quadratic function like :math:`f(n) = 5n^2 + 2`.
...
If the function that describes the work for an algorithm has multiple terms, we classify it only according to its fastest-growing term. :math:`f(n) = n^2 + 2n` would be considered **Quadratic** because for large values of **n** the :math:`n^2` will dominate. Say **n** is 100: :math:`n^2 = 10,000` and :math:`2n = 200`... the extra 200 hardly even matters compared to the 10,000.